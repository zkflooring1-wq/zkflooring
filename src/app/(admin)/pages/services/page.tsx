...
export default function ServicesPageEditor() {
  const slug = "services";
  const [data, setData] = useState<PageData>({ title: "", subtitle: "", description: "", seoTitle: "", seoDescription: "" });
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/pages/${slug}`, {
        ...
        body: JSON.stringify({
          title: "Services",
          sections: { title: data.title, subtitle: data.subtitle, description: data.description },
  ...
  if (loading) return <AdminLayout title="Services Page" breadcrumb={["Pages", "Services"]}><LoadingState /></AdminLayout>;
